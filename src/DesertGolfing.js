BasicGame.DesertGolfing = function (game) {
...
BasicGame.DesertGolfing.prototype = new Phaser.State();

BasicGame.DesertGolfing.prototype.parent = Phaser.State;
...
BasicGame.DesertGolfing.prototype = {
  ...
  create: function () {
    ...
    this.state = DesertGolfingState.START;
    ...
    // flag_left
    this.flagLeft = this.game.add.sprite(0,0,'flag_left');
    this.flagRight = this.game.add.sprite(0,0,'flag_right');
    ...
    this.hole = 1;
    this.holeTextBG = this.game.add.sprite(0,0,'white_pixel');
    this.holeTextBG.tint = 0xFFFF84;
    this.holeText = this.game.add.bitmapText(0, 0, 'atari','1024',12);
    this.holeText.tint = 0x333333;
  ...
  update: function () {
    ...
    if (this.state == DesertGolfingState.PLAY) {
  ...
  handleInput: function () {
    ...
    if (this.state == DesertGolfingState.GAME_OVER || this.state == DesertGolfingState.LOST_PADDLE || !this.paddle.visible) return;
  ...
  checkBallOut: function () {
    ...
    if (this.state != DesertGolfingState.PLAY) return;
  ...
  gameOver: function () {
    ...
    this.state = DesertGolfingState.GAME_OVER;
  ...
  showGameOver: function () {
    ...
    this.state = DesertGolfingState.GAME_OVER_SCREEN;
  ...
  restartBall: function (ball,paddle) {
    ...
    this.state = DesertGolfingState.PLAY;
  ...
  resetBricks: function () {
    ...
    for (var x = startX; x < this.game.width; x += BRICK_WIDTH) {
      for (var y = startY; y < BRICKS_Y_OFFSET + MAX_BRICK_ROWS*BRICK_HEIGHT; y += BRICK_HEIGHT) {
          var brick = new Brick(this,x,y,0,0,this.brickColor,0,this.BRICK_SFX["brick_oranger"]);
          this.game.physics.enable(brick, Phaser.Physics.ARCADE);
          brick.body.immovable = true;
          this.bricks.add(brick);
          if (!flagPlaced && y == startY && (Math.random() < 0.1 || x + 6*BRICK_WIDTH > this.game.width)) {
            this.flagLeft.x = x + BRICK_WIDTH/2;
            this.flagLeft.y = y - this.flagLeft.height;
            this.holeText.text = this.hole;
            this.holeText.x = this.flagLeft.x + this.flagLeft.width;
            this.holeText.y = this.flagLeft.y + this.flagRight.height/2 - this.holeText.height/2;
            this.holeTextBG.x = this.flagLeft.x + this.flagLeft.width;
            this.holeTextBG.y = this.flagLeft.y;
            this.holeTextBG.width = this.holeText.width;
            this.holeTextBG.height = this.flagRight.height;

            this.flagRight.x = this.holeText.x + this.holeText.width;
            this.flagRight.y = this.flagLeft.y;
      ...
      if (startY >= BRICKS_Y_OFFSET + MAX_BRICK_ROWS*BRICK_HEIGHT) {
        startY -= BRICK_HEIGHT;
      }
...
BasicGame.DesertGolfing.prototype.constructor = BasicGame.DesertGolfing;
